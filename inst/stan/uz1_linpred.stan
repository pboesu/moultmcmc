...
for (i in 1:N_moult) Q[i] = Phi((moult_dates[i] - mu[i + N_old])/sigma) - Phi((moult_dates[i] - tau - mu[i + N_old])/sigma);
for (i in 1:N_new) R[i] = Phi((new_dates[i] - tau - mu[i + N_old + N_moult])/sigma);

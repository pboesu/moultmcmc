...
for (i in 1:N_old) P[i] = 1 - Phi((old_dates[i] - mu[i])/sigma);
//print(P);
for (i in 1:N_moult) Q[i] = Phi((moult_dates[i] - mu[i + N_old])/sigma) - Phi((moult_dates[i] - tau[i + N_old] - mu[i + N_old])/sigma);
//print(Q);
for (i in 1:N_new) R[i] = Phi((new_dates[i] - tau[i + N_old + N_moult] - mu[i + N_old + N_moult])/sigma);
//print(R);
//print(sum(log(P)));
//print(sum(log(Q)));
//print(sum(log(R)));

#' Bayesian inference for the Type 4 moult model
#'
#' @export
...
uz4_linpred <- function(moult_index_column, date_column, start_formula = ~1, duration_formula = ~1, sigma_formula = ~1, data, init = "auto", log_lik = TRUE,...) {
  ...
  data <- subset(data, get(moult_index_column) != 0)
  ...
  standata <- list(moult_dates  = data[[date_column]][(data[[moult_index_column]] > 0 & data[[moult_index_column]] < 1)],
                   ...
                   N_new = length(data[[date_column]][data[[moult_index_column]]==1]),#TODO: this is not robust to NA's being thrown out by model.matrix/model.frame
                   X_mu = X_mu,
                   N_pred_mu = ncol(X_mu),
                   X_tau = X_tau,
  ...
  if(init == "auto"){
    ...
    tau_start = max(10, min(standata$new_dates)-min(standata$moult_dates))
    ...
    out <- rstan::sampling(stanmodels$uz4_linpred, data = standata, init = initfunc, pars = outpars, ...)
  } else {
    out <- rstan::sampling(stanmodels$uz4_linpred, data = standata, init = init, pars = outpars, ...)
  }

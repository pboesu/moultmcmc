#' Bayesian inference for the Type 3 moult model
#'
#' @export
...
uz3_linpred <- function(moult_index_column, date_column, start_formula = ~1, duration_formula = ~1, sigma_formula = ~1, beta_sd = 0, data, init = "auto", log_lik = TRUE,...) {
  ...
  if(init == "auto"){
    ...
    out <- rstan::sampling(stanmodels$uz3_linpred, data = standata, init = initfunc, pars = outpars, ...)
  } else {
    out <- rstan::sampling(stanmodels$uz3_linpred, data = standata, init = init, pars = outpars, ...)
  }

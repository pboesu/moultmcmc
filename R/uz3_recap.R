...
uz3_linpred_recap <- function(moult_index_column, date_column, id_column, start_formula = ~1, duration_formula = ~1, sigma_formula = ~1, data, init = "auto", log_lik = TRUE,...) {
  ...
  if(init == "auto"){
    ...
    out <- rstan::stan('stan_models/uz3_linpred_individual_start_mk2.stan', data = standata, init = initfunc, pars = outpars, ...)
  } else {
    out <- rstan::stan('stan_models/uz3_linpred_individual_start_mk2.stan', data = standata, init = init, pars = outpars, ...)
  }

...
moultmcmc <- function(moult_column,
  ...
  out_struc$replicated_ids <- if (is.null(id_column)) { NA } else { subset(data.frame(index = standata$replicated_id, id = standata$individual[standata$individual_first_index]), index > 0) }

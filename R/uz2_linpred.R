...
uz2_linpred <- function(start_formula = ~1, duration_formula = ~1, sigma_formula = ~1, data, init = "auto",...) {
  ...
  if(init == "auto"){
    ...
    out <- rstan::sampling(stanmodels$uz2_linpred, data = standata, init = initfunc, pars = c('beta_mu','beta_tau','beta_sigma', 'log_lik'), ...)
  } else {
    out <- rstan::sampling(stanmodels$uz2_linpred, data = standata, init = init, pars = c('beta_mu','beta_tau','beta_sigma', 'log_lik'), ...)
  }

...
# Priors
The following priors are currently hard-coded for the intercept terms of the linear predictors on $\mu$, $\tau$, and $\sigma$, respectively, in all `moultmcmc` models:

$\mu_0 \sim \mathrm{Uniform(0,366)}$   
$\tau_0 \sim \mathrm{Uniform(0,366)}$    
$\ln(\sigma_0) \sim \mathrm{Normal(0,5)}$

Any additional regression coefficients will use the Stan default of an improper flat prior.
